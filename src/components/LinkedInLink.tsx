...
interface LinkedInLinkProps {
  ...
  className?: string
  /** Optional children to override the default content */
  children?: React.ReactNode
}

export function LinkedInLink({ name, url, className, children }: LinkedInLinkProps) {
  return (
    <a
      href={url}
      target="_blank"
    ...
    >
      <>
        <svg
          className="h-5 w-5 flex-shrink-0 fill-current text-[#0077b5] transition-colors duration-200 group-hover:text-[#005885]"
          viewBox="0 0 24 24"
          aria-hidden="true"
          role="img"
          aria-label="LinkedIn"
