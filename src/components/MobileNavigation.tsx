...
function MobileNavigationDialog({
  ...
  return (
    ...
    >
      <DialogBackdrop
        transition
        className="fixed inset-0 top-14 bg-neutral-400/20 backdrop-blur-xs data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"
      />
      ...
      <DialogPanel>
        ...
        <TransitionChild>
          <motion.div
            layoutScroll
            className="fixed top-14 bottom-0 left-0 w-full overflow-y-auto bg-white px-4 pt-6 pb-4 ring-1 shadow-lg shadow-neutral-900/10 ring-neutral-900/7.5 duration-500 ease-in-out data-closed:-translate-x-full min-[416px]:max-w-sm sm:px-6 sm:pb-10"
          >
            <Navigation />
          </motion.div>
        </TransitionChild>
...
export function MobileNavigation() {
  ...
  return (
    <IsInsideMobileNavigationContext.Provider value={true}>
      <button
        type="button"
        className="relative flex size-6 items-center justify-center rounded-md transition hover:bg-neutral-900/5"
        aria-label="Toggle navigation"
        onClick={toggle}
      >
        <span className="absolute size-12 [@media(pointer:fine)]:hidden" />
        <ToggleIcon className="w-2.5 stroke-neutral-900" />
      </button>
