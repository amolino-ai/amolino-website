...
export default async function Introduction() {
  ...
  return (
    <section id="introduction" className="scroll-mt-20">
      ...
      <div className="mt-6 rounded-2xl bg-gradient-to-br from-primary-50/50 to-white p-8 shadow-sm">
        <p className="mb-4 text-xl leading-relaxed text-neutral-600">
          {content.introParagraph.split('Chief Revenue Officers').map((part, i) => (
            i === 0 ? part : <><strong key={i}>Chief Revenue Officers</strong>{part}</>
          ))}
        </p>
        <p className="text-neutral-600">They are essential for:</p>
        <ul className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          {content.benefits.map((benefit, index) => (
            <li key={index} className="flex items-start rounded-lg bg-white p-3 shadow-sm transition-transform hover:-translate-y-0.5 hover:shadow">
              <span className="mr-3 rounded-full bg-primary-100 p-1 text-primary-700">
